...
void read(hls::stream<input_datatype> &input_qr_stream, input_datatype *qpoint, hls::stream<input_datatype> &input_sf_stream, input_datatype *tpoint){
#pragma HLS INLINE OFF
	read_stream_loop:for(int i = 0; i < PACKAGES_PER_MAX_SEQ_LEN; i++){
#pragma HLS PIPELINE
		input_qr_stream.write(qpoint[i]);
		input_sf_stream.write(tpoint[i]);
	}
}

void read_wrapper(hls::stream<input_datatype> &input_qr_stream, input_datatype *qpoint, hls::stream<input_datatype> &input_sf_stream, input_datatype *tpoint, int n_pairs){
#pragma HLS PIPELINE off
	read_loop:for(int i = 0; i < n_pairs+1; i++,qpoint+=PACKAGES_PER_MAX_SEQ_LEN,tpoint+=PACKAGES_PER_MAX_SEQ_LEN)//one additional pair as we count the info of the scoring matrix
		read(input_qr_stream, qpoint, input_sf_stream, tpoint);
}

void write_wrapper(hls::stream<int> &scores_stream, int *v_s, int n_pairs){
//#pragma HLS PIPELINE off
#pragma HLS INLINE OFF
	write_loop:for(int i = 0; i < n_pairs; i++){
#pragma HLS PIPELINE
		v_s[i]=scores_stream.read();
	}
}

void compute(int tlen, int qlen, hls::stream<input_datatype> &input_qr_stream, hls::stream<input_datatype> &input_sf_stream, int score, datatype *mat, datatype m,
		  ...
		  datatype u[MAX_TLEN>>4][1<<4], datatype v[MAX_TLEN>>4][1<<4], datatype x[MAX_TLEN][1<<4], datatype y[MAX_TLEN][1<<4], datatype s[MAX_TLEN>>4][1<<4], ap_int<128>* p, int32_t* H132, int32_t* t_32, int32_t*  tmp32,
		  datatype* d8, datatype* d8p, datatype* z8, datatype* z8p, datatype* xt18, datatype* xt18p, datatype* tmp8, datatype* tmp8p, datatype* vt18, datatype* vt18p, datatype* a8,datatype* ut8,datatype* b8,datatype* b8p,
		  ...
		  int32_t* HH, int32_t* tt,	int32_t* max_H_32, int32_t* max_t_32, int32_t* qe_32, int* off, int* off_end
//		  ,int32_t max_H, int32_t max_t, int32_t en1, int32_t i, int st, int en, int st0, int en0, int st_, int en_, datatype x1, datatype v1
...
#pragma HLS ARRAY_PARTITION variable=sf dim=1 factor=16 cyclic
	...
	for(int i=0; i < PACKAGES_PER_MAX_SEQ_LEN; i++){
		qr_input[i] = input_qr_stream.read();
		sf_input[i] = input_sf_stream.read();
	}
...
#pragma HLS PIPELINE
	...
	// as of now this is useless read_inputs(target, query, sf, qr);
...
antidiag_loop:
	for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r)
	{
		...
		ap_int<128> *pr = p + (size_t)r * n_col_ - st_; //
		off[r] = st, off_end[r] = en;

		// set score
//		if (!(flag & HW_KSW_GENERIC_SC))
//				{
//		set_scores:
//			for (t = st0; t <= en0; t += 16)
//			{
//
//				hw_loadu_si128_8(sq8, (datatype *)&sf[t]);
//				hw_loadu_si128_8(st8, (datatype *)&qrr[t]);
//				hw_cmpeq_epi8(extra1, sq8, m1_8);
//				hw_cmpeq_epi8(extra2, st8, m1_8);
//				hw_or_si128_8(mask8, extra1, extra2);
//				hw_cmpeq_epi8(tmp8, sq8, st8);
//				hw_blendv_epi8(tmp8, sc_mis_8, sc_mch_8, tmp8);
//				hw_blendv_epi8(tmp8, tmp8, sc_N_8, mask8);
//				hw_loadu_si128_8(&s[t>>4][t&15], tmp8);
//			}
//		}
//		else
//		{
//		set_scores_matrix:
//			for (t = st0; t < st0+16; t++){
//				s[t>>4][t&15] = mat[sf[t] * m + qrr[t]];
//		}
	core_loop:
		for (t = st_; t <= en_; t++)
		{
...
//			//// set score
			if (!(flag & HW_KSW_GENERIC_SC))
			{
				hw_loadu_si128_8(sq8, (datatype *)&sf[(t<<4)+(st&15)]);
				hw_loadu_si128_8(st8, (datatype *)&qrr[(t<<4)+(st&15)]);
				hw_cmpeq_epi8(extra1, sq8, m1_8);
				hw_cmpeq_epi8(extra2, st8, m1_8);
				hw_or_si128_8(mask8, extra1, extra2);
				hw_cmpeq_epi8(tmp8, sq8, st8);
		...
		#pragma HLS UNROLL
			...
			hw_cmpgt_epi8(d8p, a8, z8);
//			hw_and_si128_8(d8, d8p, flag1_8);                          // z = z > a? z : a (signed)
			hw_and_si128_8_1(d8, d8p);
			hw_max_epi8(z8, z8, a8);
			hw_cmpgt_epi8(tmp8, b8, z8); // d = b > z? 2 : d
			hw_mask_with_two(d8, d8, tmp8);
			...
			hw_and_si128_8_8(d8p, tmp8);
			hw_or_si128_8(d8, d8, d8p);
			hw_cmpgt_epi8_zero(tmp8, b8);
			hw_and_si128_8(tmp8p, tmp8, b8);
			hw_loadu_si128_8(y[t], tmp8p);
			hw_and_si128_8_16(tmp8p, tmp8);
			hw_or_si128_8(d8, d8, tmp8p);
			hw_loadu_si128_8((datatype *)&pr[t], d8);
		}

		int32_t max_H, max_t;
		int32_t en1 = st0 + ((en0 - st0) >> 2 << 2), i;

		// compute H[], max_H and max_t
		if (r > 0)
		{

			max_H = H[en0>>4][en0&15] = en0 > 0 ? to_matrix(H,en0-1) + to_matrix(u,en0) - qe : to_matrix(H,en0) + to_matrix(v,en0) - qe; // special casing the last element
			max_t = en0;

			hw_set1_epi32(max_H_32, max_H);
			hw_set1_epi32(max_t_32, max_t);
			hw_set1_epi32(qe_32, q + e);

		find_score_adding_matrixes_diff:
			for (t = st0; t < en1; t += 4)
			{ // this implements: H[t]+=v8[t]-qe; if(H[t]>max_H) max_H=H[t],max_t=t;
#pragma HLS PIPELINE
				hw_add_sub_epi32_32_8(&H[t>>4][t&(15)], qe_32, &v[t>>4][t&15]);
//				hw_sub_epi32(&H[t>>4][t&(15)], &H[t>>4][t&(15)], qe_32);
				hw_set1_epi32(t_32, t);
				hw_cmpgt_epi32(tmp32, H132, max_H_32);
				hw_blendv_epi8((datatype *)max_H_32, (datatype *)max_H_32, (datatype *)H132, (datatype *)tmp32);
				hw_blendv_epi8((datatype *)max_t_32, (datatype *)max_t_32, (datatype *)t_32, (datatype *)tmp32);
			}

			hw_loadu_si128_8((datatype *)HH, (datatype *)max_H_32);
			hw_loadu_si128_8((datatype *)tt, (datatype *)max_t_32);
		max:
			for (i = 0; i < 4; ++i)
			{
#pragma HLS PIPELINE
			...
			//cover the last step for max and score compute (this is 3 cycles at most)
			for(;t<en0;t++){
#pragma HLS PIPELINE
				int tmp = (int32_t)to_matrix(v,t)-qe;
				H[t>>4][t&15]+=tmp;
				if (tmp > max_H)
					max_H = tmp, max_t = t;
			}
			...
			H[0][0] = to_matrix(v,0) - qe - qe, max_H = to_matrix(H,0), max_t = 0; // special casing r==0
							 // update ez
		if (en0 == tlen - 1 && to_matrix(H,en0) > ez_local.mte)
			ez_local.mte = to_matrix(H,en0), ez_local.mte_q = r - en;
		if (r - st0 == qlen - 1 && to_matrix(H,st0) > ez_local.mqe)
			ez_local.mqe = to_matrix(H,st0), ez_local.mqe_t = st0;
...
void compute_wrapper(int tlen, int qlen, hls::stream<input_datatype> &input_qr_stream, hls::stream<input_datatype> &input_sf_stream,
	...
	ap_int<128> p[(MAX_TLEN + MAX_QLEN - 1) * MAX_TLEN + 1];

	int32_t H132[4], t_32[4], tmp32[4];
	datatype d8[16], d8p[16], z8[16], z8p[16], xt18[16], xt18p[16], tmp8[16], tmp8p[16], vt18[16], vt18p[16], a8[16], ut8[16], b8[16], b8p[16];	ksw_hw_extz_t ez_local;
...
#pragma HLS ARRAY_PARTITION variable=H132 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=t_32 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=tmp32 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=d8 dim=1 complete
#pragma HLS ARRAY_PARTITION variable=d8p dim=1 complete
...
#pragma HLS ARRAY_PARTITION dim=1 variable=mat complete

	input_datatype *mat_input=(input_datatype*)mat;
	//before computing read the scoring matrix
	for(int i=0; i < PACKAGES_PER_MAX_SEQ_LEN; i++){
		if(i<2) mat_input[i] = input_qr_stream.read();
		else input_qr_stream.read();//empty garbage packets, might be used afterwards for additional info
		input_sf_stream.read();//empty garbage packets, might be used afterwards for additional info
	}

	compute_loop:for(int i = 0; i < n_pairs; i++){
#pragma HLS PIPELINE off
		compute(tlen, qlen, input_qr_stream, input_sf_stream, score, mat, m, q, e, w, zdrop, end_bonus, flag, ez_local, r, t,
						qe, n_col_, tlen_, qlen_, last_st, last_en, wl, wr, max_sc, min_sc, H, q_8, qe2_8, sc_mch_8, sc_mis_8, sc_N_8, m1_8, max_sc_8, u, v, x, y, s, p,
						H132, t_32, tmp32, d8, d8p, z8, z8p, xt18, xt18p, tmp8, tmp8p, vt18, vt18p, a8, ut8, b8, b8p, x1_32, v1_32, x1_8, v1_8, sq8, st8, mask8,
						extra1, extra2, zero, HH, tt, max_H_32, max_t_32, qe_32, off, off_end
...
void ksw_extz2_hw(int qlen, input_datatype *query_l, int tlen, input_datatype *target_l, datatype m,//, const input_datatype *mat_global,
		  datatype q, datatype e, int w, int zdrop, int end_bonus, int flag, int *v_s, int n_pairs) // ksw_hw_extz_t *ez_out)
{
//depth is only for cosim purposes, query_l and target_l account for one additional couple for scoring infos
#pragma HLS INTERFACE m_axi offset=slave bundle=gmem0 port=query_l depth=72
#pragma HLS INTERFACE m_axi offset=slave bundle=gmem1 port=target_l depth=72
#pragma HLS INTERFACE m_axi offset=slave bundle=gmem2 port=v_s depth=8

#pragma HLS INTERFACE s_axilite port=qlen bundle=control
#pragma HLS INTERFACE s_axilite port=query_l bundle=control
#pragma HLS INTERFACE s_axilite port=tlen bundle=control
#pragma HLS INTERFACE s_axilite port=target_l bundle=control
...
#pragma HLS INTERFACE s_axilite port=v_s bundle=control
...
#pragma HLS dataflow
	...
	static hls::stream<input_datatype> input_qr_stream("query_stream");
#pragma HLS STREAM variable=input_qr_stream depth=depth_stream
#pragma HLS BIND_STORAGE variable=input_qr_stream type=fifo impl=bram

	static hls::stream<input_datatype> input_sf_stream("string_stream");
#pragma HLS STREAM variable=input_sf_stream depth=depth_stream
#pragma HLS BIND_STORAGE variable=input_sf_stream type=fifo impl=bram

	static hls::stream<int> scores_stream("scores_stream");
#pragma HLS STREAM variable=scores_stream depth=depth_stream

	read_wrapper(input_qr_stream, query_l, input_sf_stream, target_l, n_pairs);


	compute_wrapper(tlen, qlen, input_qr_stream, input_sf_stream
//				, max_H, max_t, en1, i, st, en, st0, en0, st_, en_, x1, v1
				,scores_stream, n_pairs, q, e, w, zdrop, end_bonus, flag, m
				);

	write_wrapper(scores_stream, v_s, n_pairs);
