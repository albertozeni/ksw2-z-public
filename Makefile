...
HOST_SRCS += $(xcl2_SRCS)
CXXFLAGS += $(opencl_CXXFLAGS) $(SEQINCLUDE) -Wall -O3 -std=c++14 -fopenmp
...
LDCLFLAGS += --config ksw_extz2_hw.ini 
EXECUTABLE = host
CMD_ARGS = $(BUILD_DIR)/ksw_extz2_hw.xclbin
EMCONFIG_DIR = $(TEMP_DIR)
EMU_DIR = $(SDCARD)/data/emulation

BINARY_CONTAINERS += $(BUILD_DIR)/ksw_extz2_hw.xclbin
BINARY_CONTAINER_ksw_extz2_hw_OBJS += $(TEMP_DIR)/ksw_extz2_hw.xo
...
# Building kernel
$(TEMP_DIR)/ksw_extz2_hw.xo: src/ksw_extz2_hw.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k ksw_extz2_hw -I'$(<D)' -o'$@' '$<'
$(BUILD_DIR)/ksw_extz2_hw.xclbin: $(BINARY_CONTAINER_ksw_extz2_hw_OBJS)
	mkdir -p $(BUILD_DIR)
...
ifeq ($(HOST_ARCH), x86)
	...
	XCL_EMULATION_MODE=$(TARGET) ./$(EXECUTABLE) $(BUILD_DIR)/ksw_extz2_hw.xclbin
else
	mkdir -p $(EMU_DIR)
...
ifeq ($(HOST_ARCH), x86)
	./$(EXECUTABLE) $(BUILD_DIR)/ksw_extz2_hw.xclbin
endif
endif
